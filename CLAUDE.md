...
- **multipleview（多視点・カスタム）**: **cut_roasted_beef で前処理〜学習〜評価 動作確認済み（feat-013, 2026-06-25、PSNR 32.38）**。入力は「自前の多視点動画」前提で 4DGS は配布データ無し。本案件は**既存 DyNeRF `cut_roasted_beef` を multipleview 形式へ再編成して流用**（コードパスは `scene/__init__.py:61`→`readMultipleViewinfos`→`multipleview_dataset` で DyNeRF とは別物）。再編成: DyNeRF の20カメラ（cam04欠番）を **`cam01`〜`cam20` 連番**に詰め、各 `images/NNNN.png`→`camMM/frame_(NNNN+1)_5桁.jpg`（**真JPEG q95**。`cam01` 必須・全カメラ同数・連番が `multipleview_dataset` の前提）。前処理は `multipleviewprogress.sh` を**非改変のまま個別コマンドで再現**（本体改変ゼロ・uv方針順守）: `extractimages.py`（各camの`frame_00001`→`imageN.jpg`）→COLMAP（**3.11.1**、feature/matcher は `--SiftExtraction.use_gpu 0`/`--SiftMatching.use_gpu 0` で**CPU固定**、`mapper`〔完全SfM・rig非互換とは無関係〕、dense のみ `CUDA_VISIBLE_DEVICES=N`）→`sparse_`→`downsample_point.py`→`points3D_multipleview.ply`→LLFF `imgs2poses.py`（事前clone・依存 skimage/scipy/imageio は導入済みで `pip install` 不要）→`poses_bounds_multipleview.npy`(N,17)。健全性は `colmap model_analyzer`（登録20/20・再投影誤差<1.5px）で確認。config は `arguments/multipleview/cut_roasted_beef.py`（`default.py` 複製・iterations15000・`iteration_14000` 保存）。学習→`render.py --skip_train --skip_video`（multipleview の video=spiral300view は重く FR外なので skip。test=全カメラ×3時刻・held-outカメラ無し＝学習視点再構成のため PSNR は参考値）→`metrics.py`。**train/render は MPL 環境変数付与**（DyNeRF と同機構）。光源付近の floater/わずかなソフトさは 4DGS の既知特性で動作確認上は正常。詳細は `docs/issues/feat-013-multipleview/`。`data/` は `.gitignore` 管理外（git未追跡）
