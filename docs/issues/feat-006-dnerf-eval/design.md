...
設計どおりのコマンド（§1.4.1）で評価を実行し、**全 FR 合格・コード変更ゼロ**で完走した。所要時間は約62秒、exit code 0。

### 計画外対処（不具合）

なし。feat-004 で危惧した Pillow 12.2.0 の型互換問題（`dataset_readers.py:287`）は無関係だった（`readImages` は `Image.open` + `tf.to_tensor` のみで `Image.fromarray(...,"RGB")` を通らない、§1.4.2）。LPIPS 重みは ADR-4 のとおり調査段階で DL 済みで、本番では再 DL されなかった（U4）。設計どおりコードは一切変更していない。

### 算出された6指標（test 20枚平均）

| 指標 | 実測値 | 論文値（bouncingballs） | 合格閾値（§1.4.4） | 判定 |
|------|--------|------------------------|--------------------|------|
| PSNR | 40.6785 | 40.62 | ≥ 38 dB | ✓ |
| SSIM | 0.9943 | 0.9942 | ≥ 0.98 | ✓ |
| LPIPS-vgg | 0.0155 | 0.0155 | ≤ 0.05 | ✓ |
| LPIPS-alex | 0.0059 | （論文非記載） | ≤ 0.03 | ✓ |
| MS-SSIM | 0.9954 | （論文非記載） | ≥ 0.98 | ✓ |
| D-SSIM | 0.0023 | （論文非記載） | ≤ 0.01 | ✓ |

論文報告値とほぼ完全一致（特に **LPIPS-vgg=0.0155 が論文 LPIPS と同値**＝論文の LPIPS は vgg 基準と判明）。PSNR 40.68 は論文 40.62 をわずかに上回り、feat-004 実測 ITER14000=39.84 より高い（最終 iteration 20000 を正しく評価）。

### 判定結果（FR-001〜004 全合格）

| FR | 結果 | 根拠 |
|----|------|------|
| FR-001 | ✓ | exit code 0。`Scene: output/dnerf/bouncingballs/`・`Method: ours_20000`・`Metric evaluation progress: 100% 20/20`。6指標すべて数値出力。torchvision の deprecation 警告（`alexnet(True)`）は無害 |
| FR-002 | ✓ | `results.json`（224B、`ours_20000` の6指標平均）・`per_view.json`（4565B、6指標×20枚）を生成。JSON 妥当、トップレベルキーは method（`ours_20000`） |
| FR-003 | ✓ | 全指標が合格閾値クリア、論文値とほぼ一致（上表） |
| FR-004 | ✓ | `/data/sakagawa/tmp/feat006-dnerf-eval/metrics.log` にログ全体を保存 |

### 未検証事項の確定（U1〜U5）

- **U1**: 確定。exit code 0 で完走、6指標すべて数値 print
- **U2**: 確定。`results.json`/`per_view.json` 生成、`ours_20000` キー・per-view 20枚分を含む
- **U3**: 確定。全指標が §1.4.4 閾値を満たす（PSNR 40.68 ≥ 38 等）
- **U4**: 確定。ログに `Downloading:` 行なし＝LPIPS 重みはキャッシュから読込（再 DL なし）
- **U5**: 確定。所要時間 約62秒。1枚目 11.30s/it（LPIPS バックボーン vgg16/alexnet の初回構築・重み読込を含む）、2枚目以降 約2.70s/it、全体平均 3.14s/it。LPIPS を画像ごとに `LPIPS(net).to(device)` 構築するオーバーヘッドはあるが、重みはキャッシュ済みで20枚評価が約1分に収まる

### 結論

本案件の完了をもって、**「D-NeRF が正常動作する環境」の構築（学習 feat-004 → レンダリング feat-005 → 評価 feat-006）が一通り完了**した。算出指標が論文報告値とほぼ一致することから、本マシン（uv + CUDA 11.6 ビルドの CUDA 拡張 + torch 1.13.1+cu116）で 4DGaussians が論文どおりに機能していることを実証した。
