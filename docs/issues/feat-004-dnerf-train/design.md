...
設計どおり FR-001〜005 を実行。実装中に1件の不具合（Pillow 非互換）に当たり、`investigation.md` イテレーション1 で修正計画→ユーザー承認→1行修正（`scene/dataset_readers.py:287` `np.byte`→`np.uint8`）→再実行で完走。手動テスト合格・クローズ。

### 計画外対処（不具合1件）

- **bug: PIL TypeError（Pillow 12.2.0 非互換）**: 初回起動時、`scene/dataset_readers.py:287` の `Image.fromarray(np.array(arr*255.0, dtype=np.byte), "RGB")` が `TypeError: Cannot handle this data type: (1, 1, 3), |i1` でクラッシュ（データ読み込み段階）。`np.byte`=int8 を新 Pillow が "RGB"（uint8 のみ）で受理しないオリジナルコードの型誤り。**`np.byte`→`np.uint8` の1行修正**で解消（詳細・ADR は `investigation.md`）。本案件で初めて 4DGaussians オリジナルコードを変更した（CLAUDE.md 開発方針に基づき理由を案件ドキュメントに記録）

### 判定結果（FR-001〜005 全合格）

- **FR-001**: 学習起動。`Found transforms_train.json file, assuming Blender data set!` → `Generating random point cloud (2000)`（メモリ生成、設計どおり）→ `Loading Training/Test Cameras` → 学習ループ突入。CUDA 拡張（rasterizer/simple_knn）実行 OK
- **FR-002**: coarse 3000 + fine 20000 = 計23000反復を完走。`Training complete.`×1、`TypeError`/`loss is nan` =0、終了コード0
- **FR-003**: 成果物すべて生成。`output/dnerf/bouncingballs/point_cloud/iteration_20000/`（`point_cloud.ply` 6,888,242 bytes / `deformation.pth` 13.5MB / `deformation_table.pth` 28KB / `deformation_accum.pth` 334KB）＋ `cfg_args` 2,097 bytes。中間 `iteration_14000/` も生成。**coarse 保存は発生せず**（設計の saving_iterations 解析どおり）
- **FR-004**: test 評価ログ4回出力（**ITER 3000 が2回**＝coarse+fine、設計どおり）。**fine の ITER 14000 test PSNR = 39.84**（ゲート20を大幅超、論文値30+到達）。L1=0.00222
- **FR-005**: `/data/sakagawa/tmp/feat004-dnerf-train/train.log` に全ログ保存

### 未検証事項の確定（U1〜U5）

- **U1 学習時間**: 開始 15:47:00 → ITER20000保存 15:57:19、**約10分**（fine 単独 09:48、~30it/s。tqdm 実測）。「数分〜20分」の想定内
- **U2 ITER14000 test PSNR**: **39.84**（ゲート20に対し約20の余裕）
- **U3 最終点数**: 初期2000 → **27,769**（densify/prune後。上限360,000に対し十分余裕）。最終 train PSNR 45.76 / Loss 0.0016
- **U4 ポートバインド**: 6017 でバインド成功（占有なし、E1 発動せず）
- **U5 ソケット残存**: なし。プロセス終了で 6017 解放を確認
- **その他**: `events.out.tfevents.*` 非生成（tensorboard 未導入、`Tensorboard not available` 出力＝設計どおり）。`data/dnerf/bouncingballs/fused.ply` 非生成（設計どおり）
